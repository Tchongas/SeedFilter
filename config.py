...
# Versions 1.7-1.20 are supported, although with varying levels of functionality
# 1.18+ uses Xoroshiro128++ for population seed and loot generation
...
BT_INCLUSION_DIST = 1	# Maximum chunks that the minimum BTs must be in
BT_EXCLUSION_DIST = 1	# Minimum distance where maximum BTs must not be exceeded
BT_COUNT = [1, 1]		# [Minimum, maximum], both inclusive
BT_LOOT = {				# Minimum requirements for different types of loot (per BT):
	"IRON": 25,				# Amount of iron ingots   (max 32: pool of 5-8 rolls, weight 20/35, 1-4 per stack)
	"GOLD": 0,				# Amount of gold ingots   (max 32: pool of 5-8 rolls, weight 10/35, 1-4 per stack)
	"TNT": 0,				# Amount of TNT           (max 16: pool of 5-8 rolls, weight  5/35, 1-2 per stack)
	"EMERALD": 0,			# Amount of emeralds      (max 24: pool of 1-3 rolls, weight  5/15, 4-8 per stack)
	"DIAMOND": 6,			# Amount of diamonds      (max  6: pool of 1-3 rolls, weight  5/15, 1-2 per stack)
	"FOOD": 6				# Amount of fish          (max  8: 2 rolls, 2-4 per stack)
}
...
#
V_DIST = 8		# Maximum chunks away from origin
...
DT_LOOT = {			# Minimum requirements for different types of loot:
	"IRON": 3,			# Amount of iron ingots
	"GOLD": 3,			# Amount of gold ingots
	"DIAMOND": 3,		# Amount of diamonds
	"EMERALD": 3,		# Amount of emeralds
	"GUNPOWDER": 0,		# Amount of gunpowder
	"GODAPPLE": 1		# Amount of enchanted golden apples (not recommended to set this above 1)
}
