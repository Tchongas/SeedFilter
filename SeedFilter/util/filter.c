...
void clear_info(SeedInfo* info) {
	...
	info->bt_loot_gold = 0;
	info->bt_loot_tnt = 0;
	info->bt_loot_emerald = 0;
	info->bt_loot_diamond = 0;
	info->bt_loot_food = 0;
	info->pool_count = 0;
	info->ship_count = 0;
	info->ravine_count = 0;
...
int parse_filter_code(Filter* filter, char* filter_code) {
	...
	if (filter->STRONGHOLD) {
		filter->sh_mode = strtol(filter_code, &filter_code, 10); // 0 = close to spawn, 1 = FSG 150/150, 2 = custom position
		...
		filter->sh_x = strtol(filter_code, &filter_code, 10); // Custom target X (nether coords)
		filter->sh_z = strtol(filter_code, &filter_code, 10); // Custom target Z (nether coords)
		filter->sh_buffer = strtol(filter_code, &filter_code, 10); // Buffer around the custom target (nether coords)
	}
