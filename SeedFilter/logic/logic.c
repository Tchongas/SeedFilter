...
int structureFilterLogic(int64_t seed, SeedInfo* seed_info, Filter* filter) {
	...
	if (filter->RUINED_PORTAL) {
		...
		if (filter->check_rp_loot && filter->mc_version < MC_1_18) {
			if (!portal_loot(seed, seed_info, filter->rp_loot, filter->mc_version)) {
				return 0;
			}
		}
	}
	...
	if (filter->BURIED_TREASURE) {
		...
		// Pre-1.18: loot depends only on lower48 (Java LCG), check here for speed
		if (filter->check_bt_loot && filter->mc_version < MC_1_18) {
	...
	if (filter->DESERT_TEMPLE) {
		...
		if (filter->check_dt_loot && filter->mc_version < MC_1_18) {
...
int biomeFilterLogic(int64_t seed, SeedInfo* seed_info, Filter* filter, Generator *world_gen) {
	...
	// 1.18+ loot checks run BEFORE the expensive applySeed — they only need
	// the seed and positions already stored in seed_info, not the generator.
	if (filter->mc_version >= MC_1_18) {
		if (filter->BURIED_TREASURE && filter->check_bt_loot) {
			if (!buried_treasure_loot(seed, seed_info, filter->bt_minimum, filter->bt_loot, filter->mc_version)) {
				return 0;
			}
		}
		if (filter->RUINED_PORTAL && filter->check_rp_loot) {
			if (!portal_loot(seed, seed_info, filter->rp_loot, filter->mc_version)) {
				return 0;
			}
		}
		if (filter->DESERT_TEMPLE && filter->check_dt_loot) {
			if (!desert_temple_loot(seed, seed_info, filter->dt_loot, filter->mc_version)) {
				return 0;
			}
		}
		if (filter->SHIPWRECK && filter->check_ship_loot) {
	...
	if (filter->STRONGHOLD) {
		...
		} else if (filter->sh_mode == 2) {
			if (!custom_stronghold_pos(seed, filter->sh_x, filter->sh_z, filter->sh_buffer, filter->check_sh_ocean, filter->mc_version, world_gen)) {
				return 0;
			}
		} else {
	...
	if (filter->SHIPWRECK) {
		...
		if (filter->check_ship_loot && filter->mc_version < MC_1_18) {
			if (!shipwreck_loot(seed, seed_info, filter->ship_loot, filter->mc_version)) {
				return 0;
			}
		}
