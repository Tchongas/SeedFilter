...
int ruined_portal_pos(int64_t lower48, SeedInfo* seed_info, int max_dist, int mc_version) {
	...
	int64_t chunkX = rp_pos.x >> 4;
	int64_t chunkZ = rp_pos.z >> 4;
...
int lava_portal(int64_t lower48, SeedInfo* seed_info) {
	int64_t chunkX = seed_info->ruined_portal.x >> 4;
	int64_t chunkZ = seed_info->ruined_portal.z >> 4;
...
int portal_loot(int64_t lower48, SeedInfo* seed_info, RPLootRequirements* rp_loot, int mc_version) {
	Pos rp_pos = seed_info->ruined_portal;
	int64_t loot_table_seed;
	int salt = 40005;
	...
	if (mc_version >= MC_1_18) {
		...
		uint64_t popSeed = ((int64_t)rp_pos.x * carveX + (int64_t)rp_pos.z * carveZ) ^ lower48;

		Xoroshiro xr2;
		xSetSeed(&xr2, popSeed + salt);
		loot_table_seed = xNextLongJ(&xr2) ^ 0x5DEECE66DL;
	} else {
		int64_t finder_seed = lower48 ^ 0x5DEECE66DL;
		int64_t carveX = nextLong(&finder_seed) | 1;
		int64_t carveZ = nextLong(&finder_seed) | 1;

		finder_seed = ((int64_t)rp_pos.x * carveX + (int64_t)rp_pos.z * carveZ) ^ lower48 & 0xFFFFFFFFFFFFLL;
		finder_seed = (finder_seed + salt) ^ 0x5DEECE66DL;
		loot_table_seed = (int64_t)nextLong(&finder_seed) ^ 0x5DEECE66DL;
	}
