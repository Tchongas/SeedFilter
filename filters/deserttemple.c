...
int desert_temple_loot(int64_t lower48, SeedInfo* seed_info, DTLootRequirements* dt_loot, int mc_version) {
	...
	for (int dt = 0; dt < seed_info->dt_count; dt++) {
		...
		if (mc_version >= MC_1_18) {
			...
			uint64_t popSeed = ((int64_t)dt_pos.x * carveX + (int64_t)dt_pos.z * carveZ) ^ lower48;

			Xoroshiro xr2;
			xSetSeed(&xr2, popSeed + salt);
			for (int i = 0; i < 4; i++) {
				loot_table_seeds[i] = xNextLongJ(&xr2) ^ 0x5DEECE66DUL;
			}
		} else {
			int64_t finder_seed = lower48 ^ 0x5DEECE66DUL;
			int64_t carveX = nextLong(&finder_seed) | 1L;
			int64_t carveZ = nextLong(&finder_seed) | 1L;

			finder_seed = ((int64_t)dt_pos.x * carveX + (int64_t)dt_pos.z * carveZ ^ lower48) & 0xFFFFFFFFFFFFLL;
