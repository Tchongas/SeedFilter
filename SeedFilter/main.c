				...
				LOCK_THREADS;
					if (seeds_found < filter->seed_requirement) {
						...
						if (filter->BURIED_TREASURE && filter->check_bt_loot) {
							printf("  |  BT(%d,%d): Fe=%d Au=%d Dia=%d Em=%d TNT=%d Fish=%d",
								temp_info->buried_treasures[0].x,
								temp_info->buried_treasures[0].z,
								temp_info->bt_loot_iron,
								temp_info->bt_loot_gold,
								temp_info->bt_loot_diamond,
								temp_info->bt_loot_emerald,
								temp_info->bt_loot_tnt,
								temp_info->bt_loot_food);
						}
