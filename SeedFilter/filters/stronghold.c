...
// Finds if a stronghold is within buffer of the custom nether coordinates (x, z).
// Target and buffer are converted to overworld coordinates (x8).
int custom_stronghold_pos(int64_t seed, int x, int z, int buffer, int ocean_check, int mc_version, Generator* world_gen) {
	StrongholdIter sh;
	Pos _ = initFirstStronghold(&sh, mc_version, seed);

	int64_t target_x = (int64_t)x * 8;
	int64_t target_z = (int64_t)z * 8;
	int64_t buffer_ow = (int64_t)buffer * 8;
	int64_t buffer_sq = buffer_ow * buffer_ow;
	...
	for (int i = 0; i < 3; i++) {
		...
		int64_t dx = sh.pos.x - target_x;
		int64_t dz = sh.pos.z - target_z;
		if (dx * dx + dz * dz <= buffer_sq) {
			if (ocean_check) {
